...
## Sprint 57 — 徹底解決文章數量不足 + 修復無連結 bug
- [ ] 診斷：有媒體計數但無文章連結的 bug（turning_point 有 article_count 但 news_articles 無對應記錄）
- [ ] 修復：確保 turning_point 的文章連結正確存入 news_articles
- [ ] 擴充：加入台灣主要媒體 RSS（聯合、自由、中時、ETtoday、三立、TVBS、民視、蘋果、商周等）
- [ ] 擴充：多種搜尋變體（原始查詢 + 最新 + 事件 + 新聞）
- [ ] 強化：Perplexity 補充邏輯確保 50 篇目標
...
## Sprint 62 — 修復 Facebook 分享縮圖 + AI 隨機論點
- [x] 診斷 SSR meta 注入失效原因（Facebook 爸蟲 UA 偉測問題）
- [x] 修復 SSR meta 注入：改為對所有非瀏覽器請求注入 OG 標籤
- [x] 修復 OG 圖片生成：改用 Satori SVG 渲染（支援中文字型），加入 CTA 文字
- [x] 後端：ai.generateShareOpinions API（根據議題生成 4 個論點供分享）
- [x] 前端：分享對話框加入「AI 幫我想論點」按鈕，點擊即展示 4 個 AI 論點選項
- [x] 前端：分享對話框加入「分享刐 Facebook」直接按鈕（藍色）
- [x] 前端：分享對話框加入 OG 圖片預覽（可展開，不論有無論點都可預覽）
