...
export const topics = mysqlTable("topics", {
  ...
  queryZh: varchar("queryZh", { length: 256 }),
  ...
  // Meltwater 整合：追蹤關鍵字與 RSS 補充源
  meltwaterKeyword: varchar("meltwaterKeyword", { length: 256 }),
  meltwaterRssUrl: varchar("meltwaterRssUrl", { length: 1024 }),
  // 最後一次被首頁掃描標記為精選的時間（null = 從未被首頁掃描選中）
  homepageFeaturedAt: timestamp("homepageFeaturedAt"),
  // Surface 歸屬：null / 'global' = 總覽；'politics' = 政治選舉；'brand' = 品牌；'invest' = 投資
  surface: varchar("surface", { length: 32 }),
  // Opview 關鍵字監控設定（供 opviewIngestion.ts 讀取）
  opviewKeyword: varchar("opviewKeyword", { length: 512 }),
  // 最後一次從 Opview 拉資料的時間
  opviewLastFetchedAt: timestamp("opviewLastFetchedAt"),
...
export const newsArticles = mysqlTable("news_articles", {
  ...
  // 議題文章查詢索引：WHERE topicId IN (...) AND publishedAt > cutoff
  // 供 enrichWithSourceMeta（首頁來源國旗）與 getTimeline（轉折點文章）使用，避免在 40萬+ 文章上全表掃描
  idxTopicPublished: index("idx_topic_published").on(table.topicId, table.publishedAt),
}));
